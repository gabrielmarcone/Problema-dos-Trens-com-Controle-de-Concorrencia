...
* Nome.............: ThreadTrem2
* Funcao...........: Criar e configurar a Thread que movimenta o trem de baixo
*************************************************************** */
package model;
...
public class ThreadTrem2 extends Thread {
  ...
  private int processoAtual = 1; // Variavel do processo atual (para o trem cinza = 0, para o trem preto = 1)
  ...
  /* ***************************************************************
  * Metodo: ThreadTrem2 (Construtor)
  * Funcao: Inicializar um objeto da classe ThreadTrem2, definindo sua imagem e posicao inicial.
  * Parametros: ImageView imagemTrem: A imagem representando o trem, int posicao: Suas coordenadas no layout.
  * Retorno: void
  *************************************************************** */
  public ThreadTrem2(ImageView imagemTrem, Slider sliderTrem2, int posicao, int solucao) {
    this.imagemTrem = imagemTrem;
    this.posicao = posicao;
    this.sliderTrem2 = sliderTrem2; // Inicializa o slider para controlar a velocidade do trem
    ...
    switch (solucao) {
      case 0: { // Variavel de Travamento
        if (ControllerScene2.variavelDeTravamento1 == 1) {
          ...
          regiaoCritica1 = true; // Marca que o trem preto esta na regiao critica 1
          return true; // Retorna true indicando que o trem preto pode acessar a regiao critica 1
        }
      }
      case 1: { // Estrita Alternancia
        if (ControllerScene2.turno1EA == 0) {
      ...
      case 2: { // Solucao de Peterson
        ...
        if (ControllerScene2.turno1SP == processoAtual && ControllerScene2.interesseRegiao1[outroProcesso] == true) {
            ...
            regiaoCritica1 = true; // Marca que o trem preto esta na zona critica 1
            // Retorna true que o trem preto pode acessar a zona critica 1
            return true;
        }
      }
      default: {
  ...
  *************************************************************** */
  public void sairRegiaoCritica1(int solucao) {
    switch (solucao) {
      ...
      case 1: { // Estrita Alternancia
        ControllerScene2.turno1EA = 0; // Define que eh o outro turno do trem preto
  ...
  *************************************************************** */
  public boolean entrarRegiaoCritica2(int solucao) {
    switch (solucao) {
      ...
      case 1: { // Estrita Alternancia
        if (ControllerScene2.turno2EA == 0) {
          // Retorna falso se for o turno do outro trem
          return false; 
        } else {
      ...
      case 2: { // Solucao de Peterson
        ...
        if (ControllerScene2.turno2SP == processoAtual && ControllerScene2.interesseRegiao2[outroProcesso] == true) {
            ...
            regiaoCritica2 = true; // Marca que o trem preto esta na zona critica 2
            // Retorna true que o trem preto pode acessar a zona critica 2
            return true;
        }
      }
      default: {
  ...
  *************************************************************** */
  public void sairRegiaoCritica2(int solucao) {
    switch (solucao) {
      ...
      case 1: { // Estrita Alternancia
        ControllerScene2.turno2EA = 0; // Define que eh o outro turno do trem preto
  ...
  * Parametros: int posicao: A posicao atual do trem (1 ou 3).
  * Retorno: void
  *************************************************************** */
  public void animacaoTrem(int posicao) {
    switch (posicao) {
      case 1: {
        // Inicio da animacao do trem 2
        ...
        } // Movimentacao do trem 2 na primeira secao de trilhos duplos
        ...
        /* **************************** FIM REGIAO CRITICA 1 *********************************** */
          ...
          this.getImagemTrem().setRotate(0);
          this.getImagemTrem().setLayoutX(this.getImagemTrem().getLayoutX() + this.getVelocidadeX());
        } // Movimentacao do trem 2 na segunda secao de trilhos duplos
        ...
        /* **************************** FIM REGIAO CRITICA 2 *********************************** */
          ...
          this.getImagemTrem().setRotate(0);
          this.getImagemTrem().setLayoutX(this.getImagemTrem().getLayoutX() + this.getVelocidadeX());
        } // Ultima movimentacao do trem 2 na terceira secao de trilhos duplos
        else {
          this.getImagemTrem().setLayoutX(-60);
          this.getImagemTrem().setLayoutY(166);
          this.getImagemTrem().setRotate(0);
        } // Volta o trem 2 para a posicao inicial
        // Fim da animacao do trem 2
        break;
      }
      case 3: {
        // Inicio da animacao do trem 2
        if (this.getImagemTrem().getLayoutX() <= 602 && this.getImagemTrem().getLayoutX() > 475) {
          this.getImagemTrem().setLayoutX(this.getImagemTrem().getLayoutX() - this.getVelocidadeX());
        } // Primeira movimentacao do trem 2 na primeira secao de trilhos duplos
        ...
        /* **************************** INICIO REGIAO CRITICA 2 ******************************** */
          ...
          this.getImagemTrem().setLayoutY(this.getImagemTrem().getLayoutY() + this.getVelocidadeY());
          this.getImagemTrem().setLayoutX(this.getImagemTrem().getLayoutX() - this.getVelocidadeX());
        } // Primeira curva do trem 2 para baixo (Fim da regiao critica 2)
        /* **************************** FIM REGIAO CRITICA 2 *********************************** */
        ...
        /* **************************** INICIO REGIAO CRITICA 1 ******************************** */
          ...
          this.getImagemTrem().setLayoutY(this.getImagemTrem().getLayoutY() + this.getVelocidadeY());
          this.getImagemTrem().setLayoutX(this.getImagemTrem().getLayoutX() - this.getVelocidadeX());
        } // Segunda curva do trem 2 para baixo (Fim da regiao critica 1)
        /* **************************** FIM REGIAO CRITICA 1 *********************************** */
          ...
          this.getImagemTrem().setLayoutY(168);
          this.getImagemTrem().setRotate(0);
        } // Volta o trem 2 para a posicao inicial
        // Fim da animacao do trem 2
        break;
  ...
  *************************************************************** */
  ...
  public void run() {
    while (!this.isInterrupted()) { // Loop infinito para atualizar a posicao do trem
      ...
      setVelocidadeX(sliderTrem2.getValue()); // Atualiza a velocidade do trem no eixo X
      setVelocidadeY(sliderTrem2.getValue()); // Atualiza a velocidade do trem no eixo Y
      ...
      try {
        ...
        System.out.println("Thread 2 finalizada com sucesso."); // Mensagem de sucesso ao finalizar a thread
